...
public class Notification {
	...
	public static void popupWindowInvalidValueLessThenZero(String title, String message) {
		Notifications note = Notifications.create().title(title).text(message).graphic(null)
	...
	public static void popupWindowInvalidGreaterValue(String title, String message) {
		Notifications note = Notifications.create().title("Invalid Input")
				.text("Receive Value Cannot Be Greater Then Issue Value").graphic(null).hideAfter(Duration.seconds(5))
	...
	public static void mainWindowProductIDAlreadyExist() {
		Notifications note = Notifications.create().title("Product ID alread Present")
				.text("Product ID is already added. Click on Edit Selection Button for Changes").graphic(null)
				.hideAfter(Duration.seconds(5)).position(Pos.BOTTOM_RIGHT).onAction(new EventHandler<ActionEvent>() {
	...
	public static void mainWindowInvalidBillDate() {
		Notifications note = Notifications.create().title("Select Date").text("Bill Date Selected Is not Valid")
	...
	public static void invalidDateFromUser() {
		Notifications note = Notifications.create().title("Invalid Date")
				.text("Please Input the correct date in right format").graphic(null).hideAfter(Duration.seconds(5))
				.position(Pos.BOTTOM_RIGHT).onAction(new EventHandler<ActionEvent>() {
	...
	public static void nothingIsSelectedNotification() {
		Notifications note = Notifications.create().title("No Row is Selected")
				.text("Please Select a Row before deleteing and editing anything").graphic(null)
				.hideAfter(Duration.seconds(5)).position(Pos.BOTTOM_RIGHT).onAction(new EventHandler<ActionEvent>() {
	...
	public static void someExceptionOccured(String title, String message) {
		Notifications note = Notifications.create().title(title).text(message).graphic(null)
				.hideAfter(Duration.seconds(20)).position(Pos.CENTER).onAction(new EventHandler<ActionEvent>() {
